...
const float Kv = -0.02; // scales the force of dampening by the lateral fans around the target orientation
const float deadband = 2.5; // tolerance (degrees) of current orientation compared to target orientation
const float saturation = 180; // the plateau of the correction force due to maximizing lateral fan output
...
// rotation rate initialization
float rotation_rate; 
// sets initial lift magnitude to zero
...
void set_lateral_motor_magnitudes(int16_t magnitudes[3]) {
  for (int i = 0; i < 3; i++) {     //cycles through array
    if (magnitudes[i] > 0) {        //when the lateral fans are accelerating backwards
      magnitudes[i] *= 2;           // increase the magnitude of the fans to comepensate 
    }
...
/*
  ...
  Inputs:
  ddx = desired acceleration in the x direction
  ddy = desired acceleration in the y direction
  ddtheta desired acceleration in the theta direciton
...
/*
  ...
  Inputs: 
  theta_goal = Desired rotation (theta) location
  theta = Current rotation (theta) location
...
float compute_rotation_error(float theta_goal, float theta) {
  ...
  if (err < -180) {          // if degrees from desired rotation location is beyond half way in the negative direction
    err += 360;              // adds 360 in order to set the error closest to the desired location
  } else if (err > 180) {    // if degrees from desired location is beyong 180 in the positive direction
    err -= 360;              // subtracts 360 in order to set the error closest to the desired location
  }
  return err;
}
