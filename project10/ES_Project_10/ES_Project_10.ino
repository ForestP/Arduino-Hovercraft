...
const float KLv = -2100.0; // scales the force of correction to move
const float Kp = 1.1; // scales the force of correction by the lateral fans
...
// stores the modified error in theta
...
// stores b switch initial position
int B_switchPos;
// stores how many times weve moved forward
int forwardCount = 0;
// stores how many right turns are made
int rightTurnCount = 0;
// stores how many left turns are made
int leftTurnCount = 0;
// stores the postion of the craft after phase 1 (A = 0, B = 1)
int phase1Pos;
...
void set_lift_motor_magnitude(int16_t magnitude) {
  magnitude  =  clip(magnitude, 0, 165); // clips value to acceptable range of 0-153 (60%)
...
void control_step()
{
  ...
  float theta_error = compute_rotation_error(theta_goal, IMU.yaw); // calculate theta error
  display_heading(theta_error * 10); // set the heading display
  modified_error = deadband_and_saturation(theta_error, deadband, saturation); // calculate the error with deadband/saturation
...
void fsm_step() {
  ...
  static int missionPhase = 1;
  static phase2Path path;
  int turnL = 80;
  int turnR = 120;
  static int frontWallRange = 55;
  ...
  switch (state) {
    case STATE_START:
      ...
      if (GPIOE_PDIR & 0x4000000) {
        ...
        if (GPIOC_PDIR & 0x10) {
          B_switchPos = 1; // SET B SWITCH POSITION
        } else {
          B_switchPos = 0; // SET B SWITCH POSITION
        }
    ...
    case STATE_RAMP_LIFT:
      ...
      if (counter > 500) {
        ...
        if (abs(rotation_rate) > 45) {
          // its hovering!
          velocity_goal[0] = 0.06; // start moving forward
          state = STATE_FORWARD; // next state
        }
      }
      break;
    case STATE_FORWARD:
      if (calibrated_distances[0] < frontWallRange) {
        forwardCount += 1; // add to forward count
        // Stop! at wall
        velocity_goal[0] = 0.0; // stop
        set_lift_motor_magnitude(lift_magnitude - 100); // lower motor
        // set next state to go to
        if (missionPhase == 2 && path == FORWARD_PATH) {
          state = STATE_STOPPED;
        } else if (missionPhase == 2 && forwardCount == 2) {
          state = STATE_STOPPED;
        } else {
          state = STATE_STOPPING;
          set_lift_motor_magnitude(lift_magnitude); // raise motor back up
        }
      }
      break;
    case STATE_STOPPING:
      ...
      if (forwardCount < 3 ) {
        // check right sensor
        if (calibrated_distances[1] < 80) {
          // wall to the right
          // set new theta goal to turn left
          theta_goal = theta_goal + turnL;
          ...
          // set new theta goal to turn right
          theta_goal = theta_goal - turnR;
          // correct to keep in bounds
          if (theta_goal <= -180.0) {
            theta_goal += 360;
          }
      ...
      } else if (forwardCount == 3 && missionPhase == 1) {
        // time to turn around
        state = STATE_TURN_180;
        theta_goal = theta_goal + 170;
        // correct to keep in bounds
        if (theta_goal <= -180.0) {
          theta_goal += 360;
    ...
    case STATE_TURN_RIGHT:
      if (abs(modified_error) == 0) {
        // desired rotation reached!
        rightTurnCount += 1; // add to right turn count
        velocity_goal[0] = 0.06; // start moving forward
        velocity_goal[1] = -0.005; // fix drift
    ...
    case STATE_TURN_LEFT:
      if (abs(modified_error) == 0) {
        // desired rotation reached!
        leftTurnCount += 1; // add to right turn count
        int totalTurnCount = leftTurnCount + rightTurnCount;
        if (totalTurnCount == 2 && calibrated_distances[0] < 50) {
          // two turns and wall to the front, turn left again at A or B from C or D
          theta_goal = theta_goal + turnL + 10;   // set new theta goal to turn left again
          // correct to keep in bounds
          if (theta_goal >= 180) {
            theta_goal -= 360;
          }
          // set next state
          state = STATE_TURN_LEFT;
        } else if (totalTurnCount  == 3 && missionPhase == 1) {
          // three turns go to phase 2!
          state = STATE_PREPARE_PHASE_2;
        } else {
          // none of special conditions met, just go forward again
          velocity_goal[0] = 0.06; // start moving forward
          state = STATE_FORWARD; // forward state
        }
    ...
    case STATE_TURN_180:
      ...
      if (abs(modified_error) == 0) {
        // desired rotation reached!
        state = STATE_PREPARE_PHASE_2; // go prepare for phase 2!
      }
      break;
    case STATE_PREPARE_PHASE_2:
      // set mission phase to 2
      missionPhase = 2;
      // determine current craft position
      phase1Pos = determineCurrentPos();
      // reset forward count
      forwardCount = 0;
      // adjust front wall range
      frontWallRange = 30;
      // set next goal
      if (phase1Pos == 0 && B_switchPos == 1) {
        path = A_TO_D;
      } else if (phase1Pos == 1 && B_switchPos == 0) {
        path = B_TO_C;
      } else {
        path = FORWARD_PATH;
      }
      velocity_goal[0] = 0.06; // start moving forward
      state = STATE_FORWARD; // sets next state
...
/*
  Function Name: determineCurrentPos

  inputs: none

  outputs: the current position as an int
  A == 0
  B == 1

  Description: based on global variables, determines the current position and returns as an int where A == 0 and B == 1
*/

int determineCurrentPos() {
  if (rightTurnCount == 0) {
    // no right turns, at A
    return 0;
  } else {
    // right turns made , at B
    return 1;
  }
}
